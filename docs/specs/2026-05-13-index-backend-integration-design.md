...
- **§6.3 amendment:** `CodexBridgeLLM` adapter is **NO LONGER NEEDED for the add path** since we never call `infer=True`. Mem0 still requires SOME LLM provider registered at init even if unused, so register a minimal no-op via Phase 0's verified class-path-string API: `LlmFactory.register_provider("sspower-noop", "sspower_mem.mem.noop.NoOpLLM", BaseLlmConfig)`. The class itself raises `RuntimeError("infer=True forbidden in Phase C")` if ever invoked. Defensive: if a future change accidentally re-enables `infer=True`, the no-op raises loudly instead of silently activating entity-store. (See §6.3 body for the canonical example.)
- **§6.3 amendment (embedder):** Per Phase 0 Q2, `EmbedderFactory.register_provider` does not exist and `provider_to_class` is `dict[str, str]` (path strings, not class objects). Use `EmbedderFactory.provider_to_class["sspower-model2vec"] = "sspower_mem.mem.embedder.Model2VecEmbedder"` (class-path string). This is the ONLY monkey-patch; ban any other.
...
- Wire contract (pinned): Codex CLI `--json` emits the OpenAI-shape envelope from Phase B (`{"id":"...", "object":"chat.completion", "choices":[{"index":0, "message":{"role":"assistant", "content":"<STRING>"}}, ...], "usage":{...}}`). The `choices[0].message.content` STRING parses as a JSON object `{"facts": ["fact 1", "fact 2", ...]}` — an object with exactly one key `facts` holding the array. Empty success is exactly `{"facts": []}`. Invalid JSON or missing/non-array `facts` key → step 3a fails, `extracted="skipped-failed"`, rc=10.
