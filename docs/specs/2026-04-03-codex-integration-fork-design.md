...
The sspower plugin (`obra/superpowers`) is installed via `claude-plugins-official` and auto-updates. Custom modifications (codex-gate integration) get overwritten. Local skills (`~/.claude/skills/`) can't use the `sspower:` namespace prefix, so they can't be referenced by other sspower skills. A project-scoped memory was used as a workaround to remind Claude to invoke codex-gate — this is fragile and doesn't scale across projects.

## Solution

Fork `obra/superpowers` into a private repo (`sskys18/claude-skills`), add codex-gate as a native skill, and modify 3 existing skills to integrate Codex at 3 points in the workflow. Register as a separate marketplace (`codex-integration`) so the fork replaces the official sspower plugin without renaming.
...
| Field | Value |
|-------|-------|
| plugin.json `name` | `sspower` (already this value upstream — no change needed) |
| marketplace.json `name` | `codex-integration` |
| settings.json enable | `"sspower@codex-integration": true` |
| settings.json disable | `"sspower@claude-plugins-official": false` |

`plugin.json` requires zero changes — upstream already has `name: "sspower"`. This is what gives skills the `sspower:` namespace prefix. Zero internal cross-reference updates needed.
...
Per task:
    ...
    If Codex chosen:
        ...
        -> Continue sspower review cycle on Codex output
```

**Modified files:**
- `skills/executing-plans/SKILL.md` — add `sspower:codex-gate` to Integration section
- `skills/subagent-driven-development/SKILL.md` — add `sspower:codex-gate` to Integration section
...
**Modified files:**
- `skills/requesting-code-review/SKILL.md` — add cross-reference: "After review is complete, invoke `sspower:codex-gate` for independent Codex review before proceeding to `sspower:finishing-a-development-branch`"
- `skills/finishing-a-development-branch/SKILL.md` — add `sspower:codex-gate` as prerequisite in Integration section

## Updated sspower Flow
...
sspower:writing-plans
    |
sspower:using-git-worktrees
    |
sspower:executing-plans / sspower:subagent-driven-development
    -> per task: inline / subagent / Codex execute                    [Point 2]
    -> sspower:test-driven-development (within execution)
    |
sspower:requesting-code-review (Claude's review)
    |
sspower:codex-gate (Codex independent review)                     [Point 3]
    |
sspower:verification-before-completion
    |
...
| `skills/executing-plans/SKILL.md` | MODIFIED | Add `sspower:codex-gate` to Integration section |
| `skills/subagent-driven-development/SKILL.md` | MODIFIED | Add `sspower:codex-gate` to Integration section |
| `skills/requesting-code-review/SKILL.md` | MODIFIED | Add cross-reference to `sspower:codex-gate` as next step |
| `skills/finishing-a-development-branch/SKILL.md` | MODIFIED | Add `sspower:codex-gate` as prerequisite |
...
**When this activates:** During `sspower:brainstorming`, after the Claude subagent spec review loop passes, before user reviews the spec.
...
**When this activates:** Inside `sspower:subagent-driven-development` or `sspower:executing-plans`, at each task execution decision point.
...
2. If the user picks **Codex execute**:
   ...
   - Continue the sspower review cycle (spec-compliance -> code-quality) on Codex's output
3. If the user doesn't specify, default to existing sspower behavior (Claude subagent).
...
**When this activates:** After `sspower:requesting-code-review` completes, before `sspower:finishing-a-development-branch`.
...
5. Gate decision:
   - `approve` -> proceed to `sspower:finishing-a-development-branch`
...
6. Modify `skills/executing-plans/SKILL.md`: add `sspower:codex-gate` to Integration section
7. Modify `skills/subagent-driven-development/SKILL.md`: add `sspower:codex-gate` to Integration section
8. Modify `skills/requesting-code-review/SKILL.md`: add cross-reference to `sspower:codex-gate` as next step after review
9. Modify `skills/finishing-a-development-branch/SKILL.md`: add `sspower:codex-gate` as prerequisite
10. Update `.claude-plugin/marketplace.json`: set name to `codex-integration`, update owner (check current value first — upstream may use `sspower-dev` or another name)
...
13. In `~/.claude/settings.json` `enabledPlugins`, set `"sspower@claude-plugins-official": false`
14. In `~/.claude/settings.json` `enabledPlugins`, add `"sspower@codex-integration": true`
15. Restart Claude Code and verify skills load with `sspower:` prefix

## Rollback

If Phase 3 step 18 fails (skills don't load correctly):
1. In `~/.claude/settings.json`, set `"sspower@claude-plugins-official": true`
2. Remove or set `"sspower@codex-integration": false`
3. Restart Claude Code — original sspower restored
