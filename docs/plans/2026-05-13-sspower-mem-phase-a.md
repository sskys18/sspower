...
```python
"""CLI entry point. Exit codes per spec §6.1:
  0 = ok
...
def cmd_add(args) -> int:
    ...
    try:
        ...
        # digest unwritable / traversal / outside trust_root → HARD
...
def cmd_search(args) -> int:
    ...
    try:
        ...
        for s in scopes:
            if s == "project":
                ...
                print(f"sspower-mem: unknown scope: {s}", file=sys.stderr)
                return 30
    except FileNotFoundError as e:
        print(f"sspower-mem: {e}", file=sys.stderr)
        return 20
    ...
    if args.json:
        ...
        for h in hits:
            print(f"[{h['source']} {h['score']:.3f}] {h['ts']} · {h['scope']} · {h['layer']} · {h['id']}")
            print(h["content"])
            print("---")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    search.add_argument("--scope", required=True)  # project | user | project,user
    search.add_argument("--cwd")
    search.add_argument("--layer")  # comma-separated
    g = search.add_mutually_exclusive_group(required=True)
...
def main(argv: list[str]) -> int:
    ...
    except Exception as e:  # last-resort guard; preserve exit-code contract
