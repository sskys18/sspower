...
- [ ] **Step 1:** Write tests asserting: (a) `chained-shell-check.sh` denies a chained `git commit && echo` and allows a standalone `git commit > f` (use the hook directly with crafted PreToolUse JSON on stdin); (b) `auto-review.sh` round-cap: with `sspower-review-rounds-<b>` = `SSPOWER_REVIEW_MAX_ROUNDS`, the hook short-circuits (assert via log/exit, no Codex spawn — set `SSPOWER_AUTO_REVIEW` test shim or a stub bridge); (c) verdict cache hit: same diff hash twice → second is cache-served (assert via log). Each assertion isolated, no network.
