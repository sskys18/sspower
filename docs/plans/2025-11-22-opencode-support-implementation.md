...
> **For agentic workers:** REQUIRED SUB-SKILL: Use sspower:executing-plans to implement this plan task-by-task.

**Goal:** Add full sspower support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/sspower-codex` (lines 40-74)
...
- Reference: `.codex/sspower-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'sspower' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/sspower-codex` (lines 212-280)
 ...
 * (personal skills override sspower skills).
 *
 * @param {string} skillName - Name like "sspower:brainstorming" or "my-skill"
 * @param {string} sspowerDir - Path to sspower skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, sspowerDir, personalDir) {
    // Strip sspower: prefix if present
    const forcesspower = skillName.startsWith('sspower:');
    const actualSkillName = forcesspower ? skillName.replace(/^sspower:/, '') : skillName;

    // Try personal skills first (unless explicitly sspower:)
    if (!forcesspower && personalDir) {
    ...
    // Try sspower skills
    if (sspowerDir) {
        const sspowerPath = path.join(sspowerDir, actualSkillName);
        const sspowerSkillFile = path.join(sspowerPath, 'SKILL.md');
        if (fs.existsSync(sspowerSkillFile)) {
            return {
                skillFile: sspowerSkillFile,
                sourceType: 'sspower',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/sspower-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/sspower-codex` (add import at top)
...
Run: `node -c .codex/sspower-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/sspower-codex
...
**Files:**
- Modify: `.codex/sspower-codex` (lines 40-74)
...
Run: `.codex/sspower-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/sspower-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/sspower-codex` (lines 16-38, approximately)
...
Run: `.codex/sspower-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/sspower.js`
...
/**
 * sspower plugin for OpenCode.ai
 *
...
const sspowerSkillsDir = path.join(homeDir, '.config/opencode/sspower/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const sspowerPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/sspower.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/sspower.js
...
**Files:**
- Modify: `.opencode/plugin/sspower.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "sspower:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > sspower)
...
Run: `node -c .opencode/plugin/sspower.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the sspower and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const sspowerSkills = skillsCore.findSkillsInDir(
            sspowerSkillsDir,
            'sspower',
            3
          );
          ...
          const allSkills = [...personalSkills, ...sspowerSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install sspower skills to ~/.config/opencode/sspower/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'sspower:';
...
```javascript
    'session.started': async () => {
      // Read using-sspower skill content
      const usingsspowerPath = skillsCore.resolveSkillPath(
        'using-sspower',
        sspowerSkillsDir,
        personalSkillsDir
      );

      let usingsspowerContent = '';
      if (usingsspowerPath) {
        const fullContent = fs.readFileSync(usingsspowerPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingsspowerContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- sspower skills: \`sspower:skill-name\` (from ~/.config/opencode/sspower/skills/)
- Personal skills: \`skill-name\` (from ~/.config/opencode/skills/)
- Personal skills override sspower skills when names match
`;

      // Check for updates (non-blocking)
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/sspower')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/sspower && git pull` to update sspower.' :
        '';
...
**Below is the full content of your 'sspower:using-sspower' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```markdown
# Installing sspower for OpenCode
...
### 1. Install sspower Skills

```bash
# Clone sspower skills to OpenCode config directory
mkdir -p ~/.config/opencode/sspower
git clone https://github.com/sskys18/sspower.git ~/.config/opencode/sspower
```

### 2. Install the Plugin

The plugin is included in the sspower repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/sspower/.opencode/plugin/sspower.js`
...
ln -s ~/.config/opencode/sspower/.opencode/plugin/sspower.js .opencode/plugin/sspower.js
```
...
```
use use_skill tool with skill_name: "sspower:brainstorming"
```
...
Personal skills override sspower skills with the same name.

## Updating

```bash
cd ~/.config/opencode/sspower
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/sspower/.opencode/plugin/sspower.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/sspower/skills`
...
- Documentation: https://github.com/sskys18/sspower
```
...
sspower works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/sspower-codex use-skill sspower:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/sspower-codex bootstrap | head -30`
...
ls -l .opencode/plugin/sspower.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/sspower.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/sspower-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/sspower-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/sspower.js` created with tools and hooks
