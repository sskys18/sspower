...
- `git push` / `gh pr create` / `gh pr ready` / `gh pr merge` / `git merge` trigger an auto-review hook that blocks unless Codex verdict is `approve` or `approve-with-followups`. Bypass: `SSPOWER_AUTO_REVIEW=off` (emergencies only). Three SKILL.md HARD-GATEs (writing-plans, subagent-driven-development, finishing-a-development-branch) enforce the same review at earlier checkpoints
