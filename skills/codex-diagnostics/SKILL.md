...
description: Examine the sspower Codex bridge log at ~/.claude/sspower/codex.log to diagnose bridge/hook failures, spot recurring patterns, and propose targeted patches. Trigger when user says "examine codex log", "codex diagnostics", "why is codex failing", "debug bridge", "check codex log", "codex errors", or invokes /codex-diagnostics.
user_invocable: true
...
The bridge + hooks append errors/warnings to `~/.claude/sspower/codex.log`. This skill reads that log, groups patterns, and proposes concrete patches to `scripts/codex-bridge.mjs` or the bash hooks under `hooks/`.

## Log format

One line per event:
```
2026-04-23T14:22:01Z [error] bridge.review kind="schema_parse_fail" session="..." raw_preview="..."
2026-04-23T14:22:33Z [warn]  hook.auto-review kind=codex_timeout_allow dur=90s branch=...
2026-04-23T14:22:50Z [info]  hook.auto-spec-gate kind=approved dur=18s ...
```

Fields after source are space-separated `key="value"` pairs. Sources seen:
- `bridge.die` — fatal bridge errors (die() path)
- `bridge.<subcommand>` — runtime errors from implement/review/resume/etc.
- `bridge.auto_commit` — auto-commit failures
- `hook.auto-review` — push/PR/merge gate outcomes
- `hook.auto-spec-gate` — plan-commit gate outcomes (when wired)
...
- "why is auto-review timing out?" → filter `grep codex_timeout_allow "$LOG"` + show duration distribution
