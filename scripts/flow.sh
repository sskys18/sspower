...
render_orders() {
  ...
  case "$s" in
    brainstorm)
      printf 'FLOW[brainstorm %d/%d] - task: "%s". Stage = BRAINSTORM. Invoke sspower:brainstorming. Produce a reviewed design doc - no implementation code. When the doc exists, run: bash "%s" set-design <path>, run codex plan-review on it, then: bash "%s" set-design-review approve, then: bash "%s" advance. Gate to advance: design doc recorded + design-review approved.' \
        "$n" "$t" "$task" "$FLOW_SH" "$FLOW_SH" "$FLOW_SH" ;;
    plan)
      printf 'FLOW[plan %d/%d] - task: "%s". Stage = PLAN. Invoke the sspower:writing-plans skill. Produce the plan only - no implementation code. When the plan file exists, run: bash "%s" set-plan <path-to-plan>, then bash "%s" advance. Auto-advance is on - drive the whole pipeline yourself, stop only on failure. Gate to advance: plan file recorded (flow set-plan).' \
        "$n" "$t" "$task" "$FLOW_SH" "$FLOW_SH" ;;
    plan-review)
      printf 'FLOW[plan-review %d/%d] - task: "%s".%s Stage = PLAN-REVIEW. Review the plan: run `node "%s/scripts/codex-bridge.mjs" plan-review --cd "%s" --prompt @%s`. Fix every high/medium finding inline and re-run until the verdict is approve or approve-with-followups, then: bash "%s" set-plan-review approve, then: bash "%s" advance. Gate to advance: plan-review marker approved (flow set-plan-review approve). Optional: bash "%s" enter-worktree <path> to isolate before exec.' \
        "$n" "$t" "$task" "$ref" "$PLUGIN_ROOT" "$CWD" "${plan_path:-<plan-path>}" "$FLOW_SH" "$FLOW_SH" "$FLOW_SH" ;;
    exec)
      printf 'FLOW[exec %d/%d] - task: "%s".%s Stage = EXEC. Plan approved. Invoke sspower:executing-plans (or test-driven-development for a feature/bugfix) and implement the plan. If the work is fan-out-shaped (many independent files/sites, >=8), first invoke sspower:dispatching-parallel-agents to route - small batches via the Task tool, large (>=8 units) or verification-heavy work via sspower:orchestrating-workflows (a native Workflow); keep git out of fan-out agents. When implementation is complete, run: bash "%s" advance. Gate to advance: implementation complete.' \
        "$n" "$t" "$task" "$ref" "$FLOW_SH" ;;
    test)
      printf 'FLOW[test %d/%d] - task: "%s". Stage = TEST. Invoke sspower:verification-before-completion. Run tests, type-check, lint - real commands, real output. PASS -> run: bash "%s" advance. FAIL -> run: bash "%s" back (returns to EXEC), fix, retry. Never advance with failures outstanding. Gate to advance: tests pass (soft gate warns if no test-result.json).' \
        "$n" "$t" "$task" "$FLOW_SH" "$FLOW_SH" ;;
    review)
      printf 'FLOW[review %d/%d] - task: "%s". Stage = REVIEW. Invoke sspower:requesting-code-review on the diff. For a broad multi-dimension pass, sspower:orchestrating-workflows can fan out adversarial verifiers (incl. a codex lens) in the background. If a blocker needs a code change, run: bash "%s" back. When the review is clean, run: bash "%s" advance - moves to MERGE. Gate to advance: review clean.' \
        "$n" "$t" "$task" "$FLOW_SH" "$FLOW_SH" ;;
    merge)
